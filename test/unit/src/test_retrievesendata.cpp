...
TEST(RetrieveSensorData, Get_local_sensor_data_from_arduino_module)
{
    ...
    rsd.getLocalSenData(ptr_localWeatherData);

    printf("Sensor ID: %s\n", ptr_localWeatherData->weatherSensorUnion.temporaryStruct.sensorID);
    printf("Temp: %i\n", ptr_localWeatherData->weatherSensorUnion.temporaryStruct.temperature);
    printf("Hum: %i\n", ptr_localWeatherData->weatherSensorUnion.temporaryStruct.perBatt);

    EXPECT_EQ("Here", ptr_localWeatherData->weatherSensorUnion.temporaryStruct.sensorID);
}